...
if [[ -n "$TMUX_BIN" ]]; then
  "$TMUX_BIN" new-session -d -s trading-lab-backend -c "$ROOT_DIR" \
    "$PYTHON_BIN -m uvicorn backend.main:app --host '$BACKEND_HOST' --port '$BACKEND_PORT' 2>&1 | tee -a '$LOG_DIR/backend.log'"
...
if [[ -n "$TMUX_BIN" ]]; then
  "$TMUX_BIN" new-session -d -s trading-lab-frontend -c "$ROOT_DIR" \
    "VITE_API_URL='$API_URL' VITE_WS_URL='$WS_URL' conda run --no-capture-output -n '$CONDA_ENV' npm run dev -- --host '$FRONTEND_HOST' --port '$FRONTEND_PORT' 2>&1 | tee -a '$LOG_DIR/frontend.log'"
